...
void Message::setContent(const nlohmann::json& content)
{
  m_JSON["content"] = static_cast<nlohmann::json>(content);
}

void Message::setContent(const std::string& content)
{
  m_JSON["content"] = nlohmann::json::parse(content, nullptr, false);
  if(m_JSON["content"].is_discarded())
  {
    m_JSON["content"] = static_cast<std::string>(content);
  }
}
...
Message::Message(const nlohmann::json& content, const MessageType& messageType) : Message(messageType)
{
  setContent(content);
}

Message::Message(const std::string& content, const MessageType& messageType) : Message(messageType)
{
  setContent(content);
}

Message::Message(const char* content, const MessageType& messageType) : Message(messageType)
{
  setContent(content);
}

std::string Message::dump(const int& indent,const char& indent_char,const bool& ensure_ascii, const nlohmann::detail::error_handler_t& error_handler) const
{
  return m_JSON.dump(indent,indent_char,ensure_ascii,error_handler);
}

nlohmann::json Message::get() const
{
  return m_JSON;
}
...
MessageType Message::getTypeValue() const
{
  return magic_enum::enum_cast<MessageType>(m_JSON["type"].get<std::string>()).value();
}

std::string Message::getContent() const
{
  if(m_JSON["content"].is_null()) return "";
  else if (m_JSON["content"].is_string()) return m_JSON["content"].get<std::string>();
...
Identifier Message::getIdentifier() const
{
  ...
  else
  {
    ...
    id.generateKey(magic_enum::enum_cast<Domain>(m_JSON["from"]["domain"].get<std::string>()).value(),
                    magic_enum::enum_cast<Class>(m_JSON["from"]["class"].get<std::string>()).value(),
...
Message::Message(const MessageType& messageType) : Message()
{
  m_JSON["type"] = magic_enum::enum_name(messageType);
}
