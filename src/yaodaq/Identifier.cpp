...
void Identifier::generateKey(const Domain& domain, const Class& c_lass, const Family& family)
{
  m_Key = Key(domain,c_lass,family);
}

std::string Identifier::getDomain() const
{
  return static_cast<std::string>(magic_enum::enum_name(magic_enum::enum_cast<Domain>(m_Key.getDomain()).value()));
}

std::string Identifier::getClass() const
{
  return static_cast<std::string>(magic_enum::enum_name(magic_enum::enum_cast<Class>(m_Key.getClass()).value()));
}

std::string Identifier::getFamily() const
{
  return static_cast<std::string>(magic_enum::enum_name(magic_enum::enum_cast<Family>(m_Key.getFamily()).value()));
}
...
{
  return fmt::format("{0}/{1}/{2}/{3}/{4}",getDomain(),getClass(),getFamily(),getType(),getName());
}
